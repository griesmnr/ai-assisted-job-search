...
// this test controls the same six vars the same way, for the same reason
...
beforeEach(() => {
  ...
  // createXSourceFromEnv() throws synchronously.
...
describe("buildAllSources", () => {
  it("builds every configured source and skips every unconfigured one independently, without throwing", () => {
    ...
    // Exactly one adapter is configured in this test's env — the other
    // three real adapters (wa-state has no adapter at all, see
    // registry.ts's own BUILDERS comment) are unconfigured and must each
    // be reported, not swallowed or allowed to abort the loop.
